...
int main(int argc, char **argv)
{
    ...
    if (argc == 4) {
        if (strcmp(argv[1], "decrypt") == 0) { // vérifies que argv[1] soit 'decrypt'
            if (...) { // verifies que argv[3] soit un nombre
                printf("Invalid STEPS '%s', should be an integer.", argv[2]);
                return -1;
            }
            message = ... // ouvres le fichier passé en argv[2]
            if (message == NULL) {
                printf("Couldn't read file '%s'.", argv[1]);
                free(message);
                return -1;
            }
            … // appelles notre fonction decrypt avec argv[3] converti en nombre.
            free(message);
            return 0;
        }
        ... //prendre exemple sur la partie "decrypt" et faire la partie "encrypt"
